...
public class AsyncMethodProcessor implements ClassHolderTransformer {
    ...
    private void generateLowLevelCall(MethodHolder method, int suffix) {
        ...
        Variable[] argArray = suspendInvocation.getArguments().toArray(
                new Variable[suspendInvocation.getArguments().size() + 1]);
        argArray[argArray.length - 1] = newInstruction.getReceiver();
        suspendInvocation.setArguments(argArray);
    ...
    private Variable castPrimitive(BasicBlock block, Variable value, String name, ValueType type) {
        ...
        Variable[] arguments = invoke.getArguments().toArray(new Variable[invoke.getArguments().size() + 1]);
